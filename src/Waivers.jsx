...
function FavoriteCard({ item, onChange, onRemove, onOpen }) {
  ...
  return <article className="wv-favorite-card">
    <div><button className="wv-favorite-name" onClick={onOpen} disabled={!item.profileId}>{item.name}</button><button aria-label={`Remove ${item.name} from Favorites`} onClick={onRemove}><X /></button></div>

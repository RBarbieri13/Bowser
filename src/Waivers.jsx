  ...
  return <main className={`page-content waivers wv-${prefs.density}`}>
    ...
    <div className={`wv-layout${prefs.drawer ? ' with-favorites' : ''}`}>
      <section className="wv-workspace">
        ...
        <TableSettingsPanel open={settings} onClose={()=>setSettings(false)} title="Waiver table settings" columns={columns.map(column=>({...column,maxWidth:420}))} value={prefs} onChange={next=>setPrefs(old=>({...old,...next}))}/>

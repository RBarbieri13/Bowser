import { TableSettingsPanel } from './TableSettings.jsx';
...
export function Waivers({ season = 2026, onOpenPlayer }) {
  ...
  const [favoriteOnly, setFavoriteOnly] = useState(false), [settings, setSettings] = useState(false), [filters, setFilters] = useState(false);
  ...
  const groups = GROUPS.map(([key, label]) => ({ key, label, columns: columns.filter(column => column.group === key && (column.required || !prefs.hidden.includes(column.key))).sort((a,b)=>(prefs.order?.indexOf(a.key) ?? -1)-(prefs.order?.indexOf(b.key) ?? -1)) })).filter(group => group.columns.length);
  ...
  function textValue(row, column) {
    ...
    if (column.kind === 'stat' && prefs.numberFormat && prefs.numberFormat !== 'source') return formatTableValue(value,{...column,type:column.percent?'percent':'number'},prefs);
    ...
    <div className={`wv-layout${prefs.drawer ? ' with-favorites' : ''}`}>
      <section className="wv-workspace">
        <div className="wv-toolbar">
          ...
          <button aria-expanded={filters} onClick={() => setFilters(value => !value)}>Source filters{ranges.some(range => range.source && (range.min !== '' || range.max !== '')) ? ' •' : ''}</button>
          <button aria-expanded={settings} onClick={() => setSettings(value => !value)}><SlidersHorizontal />Table</button>
          <span className="wv-count" role="status">{busy ? 'Loading…' : `${rows.length} players`}</span>
        </div>
        {filters && <section className="wv-filter-panel" aria-label="Source range filters">
        ...
        <TableSettingsPanel open={settings} onClose={()=>setSettings(false)} title="Waiver table settings" columns={columns.map(column=>({...column,maxWidth:420,orderGroup:column.group,type:['rank','faab','stat','activity'].includes(column.kind)?(column.percent?'percent':'number'):'text'}))} value={prefs} onChange={next=>setPrefs(old=>({...old,...next}))}/>

...
export function DataTable({ id, title, columns, rows, rowKey, getValue=(row,key)=>row[key], renderCell, filters, defaultSorts=[], footer, className='', tableClassName='', rowLabel }) {
  ...
  const filtered = useMemo(()=>rows.filter(row=>{if(search.trim()&&!columns.some(column=>String(getValue(row,column.key)??'').toLowerCase().includes(search.trim().toLowerCase())))return false;if(rangeKey&&(minimum!==''||maximum!=='')){const value=getValue(row,rangeKey);if(typeof value!=='number'||!Number.isFinite(value))return false;if(minimum!==''&&value<Number(minimum))return false;if(maximum!==''&&value>Number(maximum))return false;}return true;}),[rows,columns,search,getValue,rangeKey,minimum,maximum]);

...
function PositionGroup({ group, metrics, onMetricChange, games, slots, domains, season, scoring, onOpenPlayer }) {
  const columns=[{key:'name',label:'Player / status',group:'Player',width:270,minWidth:180,required:true},{key:'depthRank',type:'number',label:'Depth',group:'Player',width:70},{key:'position_finish',type:'number',label:'Position finish',group:'Fantasy',width:110},{key:'draft_kings_price',label:'DK salary',group:'DFS',width:110,type:'currency'},{key:'draft_kings_projection',type:'number',label:'DK projection',group:'DFS',width:120,decimals:2},{key:'context',type:'number',label:'Recent change',group:'Trends',width:245},...metrics.map((metric,i)=>({key:`trend${i}`,type:'number',label:`Chart ${i+1}`,group:'Trends',width:300,minWidth:180})),...metrics.map((metric,i)=>({key:`average${i}`,type:'number',label:`Chart ${i+1} · last 3 avg`,group:'Averages',width:120}))];
